...
if args.mode == 'both' or args.mode == 'subscribe':
    ...
    myAWSIoTMQTTClient.subscribe("emission/all/vehicle3", 0, None)
    #print("subs")
#time.sleep(2)

loopCount = 0

with open('./data/vehicle3.csv', 'r') as f:
    next(f)
    for line in f:
        line = line.split(",")
        message = {'vid': 3, 'data': line[2]}
        messageJson = json.dumps(message)
        myAWSIoTMQTTClient.publish("emission/trigger3", messageJson, 0)
        #myAWSIoTMQTTClient.publish("hello/mo/pubsub", messageJson, 0)
        myAWSIoTMQTTClient.publish("emission/cloudDevice3", messageJson, 0)
        if args.mode == 'publish' or args.mode == 'both':
            print('Published topic %s: %s\n' % ("emission/trigger3", messageJson))
        #time.sleep(1)
